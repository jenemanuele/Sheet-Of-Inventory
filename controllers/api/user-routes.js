...
const { User, Instruement, Category, Brand, Item } = require('../../models');
...
// GET /api/user/1 -- get a single user by id
router.get('/:id', (req, res) => {
    ...
    User.findOne({
      ...
      // include the Intruments the user has created, the Brands the user has included, and the itmems the user has 
      include: [
        {
          model: Instruement,
        ...
        {
            model: category,
            attributes: ['id', 'name', 'post_id', 'user_id', 'created_at'],
            include: {
                model: Post,
                attributes: ['title']
            }
